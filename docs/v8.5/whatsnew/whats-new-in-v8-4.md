...
_build:
  list: false
aliases:
  - /docs/grafana/v8.5/guides/whats-new-in-v8-4/
  - /docs/grafana/v8.5/whatsnew/whats-new-in-v8-4/
...
You can use Grafana’s SAML integration to map organizations in your SAML service to [Organizations](​​https://grafana.com/docs/grafana/v8.5/manage-users/server-admin/server-admin-manage-orgs/) in Grafana so that users who authenticate using SAML have the right permissions. Previously, you could only choose a single role (Viewer, Editor, or Admin) for users, which would apply to all of their Organizations. Now, you can map a given SAML user or org to different roles in different Organizations, so that, for example, they can be a Viewer in one Organization and an Admin in another.
