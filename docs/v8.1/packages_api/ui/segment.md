+++
# -----------------------------------------------------------------------
# Do not edit this file. It is automatically generated by API Documenter.
# -----------------------------------------------------------------------
title = "Segment"
keywords = ["grafana","documentation","sdk","@grafana/ui"]
type = "docs"
disable_edit_link = true
+++

## Segment() function

### Segment() function

<b>Signature</b>

```typescript
export declare function Segment<T>({ options, value, onChange, Component, className, allowCustomValue, allowEmptyValue, placeholder, disabled, inputMinWidth, inputPlaceholder, onExpandedChange, autofocus, ...rest }: React.PropsWithChildren<SegmentSyncProps<T>>): JSX.Element;
```
<b>Import</b>

```typescript
import { Segment } from '@grafana/ui';
```
<b>Parameters</b>

|  Parameter | Type | Description |
|  --- | --- | --- |
|  { options, value, onChange, Component, className, allowCustomValue, allowEmptyValue, placeholder, disabled, inputMinWidth, inputPlaceholder, onExpandedChange, autofocus, ...rest } | <code>React.PropsWithChildren&lt;SegmentSyncProps&lt;T&gt;&gt;</code> |  |

<b>Returns:</b>

`JSX.Element`

